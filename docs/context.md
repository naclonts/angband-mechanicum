...
Roguelike overhead tile map. The player (@) moves through persistent dungeon levels using numpad/vi-keys. Combat stays on the same map: adjacent contacts use bump-to-attack, while `f` enters a cursor-based fire mode for visible hostile targets inside weapon range and line of sight. FOV/line-of-sight determines visibility. NPCs, items, and terrain features populate the dungeon.
...
Dungeon-to-text bridges also seed the text status panel with the live dungeon location, while pending text-bridge context is only reused when returning from a prior text response.
